...
|----|--------------|------------------|
| 1  | `src/proxy.ts` matcher narrowed to Clerk's documented form, excluding a named extension list rather than any path segment containing a dot | Vitest test over `config.matcher`: `/producers/acme.co` and `/producers/acme` are matched, `/favicon.ico` and a `/_next/static/…` asset are skipped |
