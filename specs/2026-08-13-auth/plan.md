...
| 7  | `clerkMiddleware` in `src/proxy.ts` protecting every route except `/sign-in` and Next's static assets | Unit test over the route matcher: `/`, `/record`, `/producers`, `/sites` are protected; `/sign-in`, `/_next/static/*`, and `/favicon.ico` are not |
